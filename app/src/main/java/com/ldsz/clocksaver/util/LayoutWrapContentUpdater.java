...
public class LayoutWrapContentUpdater
{
    ...
    {
        if(VERBOSE) Log.d(TAG, "+++ LayoutWrapContentUpdater wrapContentAgain on subTreeRoot=["+ subTreeRoot +"], with w="
    ...
    private static void recurseWrapContent( View nodeView, boolean relayoutAllNodes )
    {
        ...
        if ( isWrapWidth || isWrapHeight ) {
            ...
            if ( isWrapWidth  && nodeView.getMeasuredWidth() > 0 ) {
                ...
                changed = true;
                if(VERBOSE) Log.v(TAG, "+++ LayoutWrapContentUpdater recurseWrapContent set Width to "+ nodeView.getMeasuredWidth() +" of node Tag="+ nodeView.getTag() +" ["+ nodeView +"]");
            }
            if ( isWrapHeight && nodeView.getMeasuredHeight() > 0 ) {
                bottom = nodeView.getTop() + nodeView.getMeasuredHeight();
                changed = true;
                if(VERBOSE) Log.v(TAG, "+++ LayoutWrapContentUpdater recurseWrapContent set Height to "+ nodeView.getMeasuredHeight() +" of node Tag="+ nodeView.getTag() +" ["+ nodeView +"]");
            }
